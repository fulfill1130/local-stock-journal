...
The first optional implementation lives in `src/pywebview_desktop_shell.py`. `PyWebviewDesktopShell` is still demo-only, uses the embedded demo desktop server, lazy-loads or accepts an injected `webview` module, and is tested with mocked webview objects so the test suite does not require a real GUI. It can be launched with `python src/main.py desktop-demo` after installing `requirements-desktop.txt`, but it is not packaged as an `.exe` yet.
